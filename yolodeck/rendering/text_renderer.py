...
class TextRenderer:
    def __init__(self, renderer, text):
        ...
        self.text = text
        self.font_filename = os.path.abspath(
    ...
    def end(self):
        ...
        if self.center_vertical is not None:
            label_pos = (
                label_pos[0],
                ((self.renderer.img.height - label_h) // 2) +
                self.center_vertical
            )
        if self.center_horizontal is not None:
            label_pos = (
                ((self.renderer.img.width - label_w) // 2) +
                self.center_horizontal,
                label_pos[1]
            )

        draw.text(
            label_pos, text=self.text, font=font, fill=self.fill,
            align=self.align
        )
